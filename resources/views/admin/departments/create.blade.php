...
$(document).ready(function(){
 ...
 $('#jstree').on('changed.jstree',function(e,data){
    var i , j , r = [];
    for(i=0,j = data.selected.length;i < j;i++) ///if i <j then i=i+1
    {
        r.push(data.instance.get_node(data.selected[i]).id);
    }
    $('.parent_id').val(r.join(', ')); ////when i click on departmen t it saved on that hidden field of class parent_id
});
